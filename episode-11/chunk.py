...
class Chunk:
	...
	def update_mesh(self):
		...
		for subchunk_position in self.subchunks:
			...
			# print("before", subchunk.mesh_indices, self.mesh_index_counter)
			mesh_indices = [index + self.mesh_index_counter for index in subchunk.mesh_indices]
			# print("after", subchunk.mesh_indices, self.mesh_index_counter)

			self.mesh_indices.extend(mesh_indices)
			self.mesh_index_counter += subchunk.mesh_index_counter

	def send_mesh_data_to_gpu(self):
		# it's important to set this *after* we're sure our data is being passed, so that our draw function isn't accidentally called
	...
	def draw(self):
		if not self.mesh_indices_length:
			return
